...
//}
//------------------------------------------------
extern "C" void trotting_horse_lamp_interrupt_handler(){
    char str[]="23320104ltx";
    // 清空屏幕
    for (int i = 0; i < 80; ++i){
        stdio.print(0, i, ' ', 0x07);
    }
    int len=11;
    int pre_index=(times-1) > 0 ? (times - 1)%len : (times- 1 + len)%len;
    int cur_indedx=(times) > 0 ? times%len : (times + len)%len;
    if(cur_indedx == 10){
        for(int i=0;str[i];i++){
            stdio.print(str[i],i+2);
        }
        asm_delay();
    }
    times++;
    stdio.moveCursor(pre_index);
    stdio.print(' ');
    stdio.moveCursor(cur_indedx);
    stdio.print(str[cur_indedx],cur_indedx+2);
    asm_delay();
}
//---------------------------------------------------lab5
// 中断处理函数
