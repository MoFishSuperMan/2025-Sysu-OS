...
void zombie_process()
{
    ...
    if (pid)
    {
	printf("I am father, now is exitting, ret is %d\n",23320104);
	exit(23320104);
    }
    else
    {
        ...
        while (tmp)
            --tmp;
        printf("I am child, now is exitting, ret is %d, pid: %d\n",-123, programManager.running->pid);
        exit(-123);
    }
}
...
void first_thread(void *arg)
{
    ...
    programManager.executeProcess((const char *)zombie_process, 1);
