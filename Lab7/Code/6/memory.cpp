...
{
    queueHead = KERNEL_VIRTUAL_START;
    this->totalMemory = 0;
    ...
    this-> userPhysicalStartAddress = usedMemory + kernelPages * PAGE_SIZE;
...
int MemoryManager::allocatePages(enum AddressPoolType type, const int count)
{
    ...
    {
        releasePages(type, queueHead ,1);
        cnt += 1;
        int VirtualAddressSize = userPhysicalStartAddress - kernelPhysicalStartAddress;
        int VirtualAddressEnd = VirtualAddressSize + KERNEL_VIRTUAL_START;
        queueHead = (queueHead == VirtualAddressEnd) ? KERNEL_VIRTUAL_START : queueHead + PAGE_SIZE;
        virtualAddress = allocateVirtualPages(type, count);
    }
    if(cnt){
        printf("release %d page with start address 0x%x\n",cnt, queueHead-cnt * PAGE_SIZE);
    }
    bool flag;
